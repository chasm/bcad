class AutomobilesController < ApplicationController
  ...
  def index
    ...
    photos = @database["photos"].query.all.map do |x|
      {id: x.key}.merge(x.to_hash).reject {|k,v| k == "error" }
    end
    
    render :json => { automobiles: automobiles, photos: photos }
  end
  ...
  def show
    ...
    photos = @database["photos"].query.by_example({ automobile_id: params[:id] }).map do |x|
      {id: x.key}.merge(x.to_hash).reject {|k,v| k == "error" }
    end
  ...
  def update
    ...
    automobile["make"]                 = params[:automobile][:make]                 || automobile["make"]
    automobile["model"]                = params[:automobile][:model]                || automobile["model"]
    automobile["color"]                = params[:automobile][:color]                || automobile["color"]
    automobile["price"]                = params[:automobile][:price]                || automobile["price"]
    ...
    if resp["error"]
      ...
      render :json => { automobiles: automobiles, photos: photos }, :status => status
    end
  end

  def destroy
    begin
      @database["automobiles"].query.first_example({ _key: params[:id] }).each do |automobile|
        automobile.delete
      end
      
      @database["photos"].query.first_example({ automobile_id: params[:id] }).each do |photo|
        photo.delete
      end
